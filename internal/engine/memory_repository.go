...
func (r *MemoryExecutionRepository) GetStepExecution(_ context.Context, id StepExecutionID) (StepExecution,error) {
	r.mu.RLock(); defer r.mu.RUnlock(); s,ok:=r.steps[id]; if !ok{return StepExecution{},ErrNotFound}; return cloneStepExecution(s),nil
}

func (r *MemoryExecutionRepository) ListStepExecutions(_ context.Context, executionID ExecutionID) ([]StepExecution,error) {
	r.mu.RLock(); defer r.mu.RUnlock(); if _,ok:=r.executions[executionID]; !ok{return nil,ErrNotFound}
...
func (r *MemoryExecutionRepository) StartStep(_ context.Context,id StepExecutionID) error {
	r.mu.Lock(); defer r.mu.Unlock(); s,ok:=r.steps[id]; if !ok{return ErrNotFound}; if s.Status!=StepPending{return transitionError("step",string(s.Status),string(StepRunning))}
...
func (r *MemoryExecutionRepository) CancelStep(_ context.Context,id StepExecutionID) error {
	r.mu.Lock(); defer r.mu.Unlock(); s,ok:=r.steps[id]; if !ok{return ErrNotFound}; if s.Status==StepCancelled{return nil}; if s.Status!=StepPending&&s.Status!=StepRunning{return transitionError("step",string(s.Status),string(StepCancelled))}
...
func (r *MemoryExecutionRepository) finishStep(id StepExecutionID,status StepStatus,output json.RawMessage,execErr *ExecutionError,eventType ExecutionEventType) error {
	r.mu.Lock(); defer r.mu.Unlock(); s,ok:=r.steps[id]; if !ok{return ErrNotFound}; if s.Status!=StepRunning{return transitionError("step",string(s.Status),string(status))}
...
func (r *MemoryExecutionRepository) CreateStepAttempt(_ context.Context,in NewStepAttempt)(StepAttemptID,error){
	r.mu.Lock(); defer r.mu.Unlock(); s,ok:=r.steps[in.StepExecutionID]; if !ok{return "",ErrNotFound}; if s.Status!=StepRunning{return "",transitionError("step",string(s.Status),"create attempt")}
...
func (r *MemoryExecutionRepository) finishAttempt(id StepAttemptID,status StepAttemptStatus,execErr *ExecutionError) error{
	r.mu.Lock(); defer r.mu.Unlock(); a,ok:=r.attempts[id]; if !ok{return ErrNotFound}; if a.Status==status&&(status==StepAttemptCancelled){return nil}; if a.Status!=StepAttemptRunning{return transitionError("step attempt",string(a.Status),string(status))}; s:=r.steps[a.StepExecutionID]
...
func (r *MemoryExecutionRepository) ListStepAttempts(_ context.Context,stepID StepExecutionID)([]StepAttempt,error){
	r.mu.RLock(); defer r.mu.RUnlock(); if _,ok:=r.steps[stepID]; !ok{return nil,ErrNotFound}; out:=make([]StepAttempt,0); for _,a:=range r.attempts{if a.StepExecutionID==stepID{out=append(out,cloneStepAttempt(a))}}; sort.Slice(out,func(i,j int)bool{return out[i].Attempt<out[j].Attempt}); return out,nil
}
func (r *MemoryExecutionRepository) ListEvents(_ context.Context,executionID ExecutionID)([]ExecutionEvent,error){r.mu.RLock(); defer r.mu.RUnlock(); if _,ok:=r.executions[executionID]; !ok{return nil,ErrNotFound}; return append([]ExecutionEvent(nil),r.events[executionID]...),nil}
