...
`skipped` and `cancelled` have different meanings. A skipped step was deliberately not required by workflow semantics; a cancelled step did not run or complete because execution cancellation took effect.
...
The repository also owns identity creation for persisted runtime entities. Callers provide the data required to create an execution, step execution, or attempt; the repository creates and returns its persistent ID. Runtime IDs should not be passed through `context.Context`, which remains reserved for cancellation, deadlines, and request-scoped values.

A representative interface is:

```go
type ExecutionRepository interface {
    CreateExecution(context.Context, Execution) (ExecutionID, error)
    ...
    CreateStepExecution(context.Context, StepExecution) (StepExecutionID, error)
    ...
    CreateStepAttempt(context.Context, StepAttempt) (StepAttemptID, error)
...
This is a design shape rather than a frozen Go API. In particular, creation inputs may later use dedicated types such as `NewExecution`, `NewStepExecution`, and `NewStepAttempt` so callers cannot supply an ID field that conflicts with repository ownership. Error return values and whether some operations are combined may also evolve while implementing the in-memory repository.

This ownership rule applies to runtime persistence identities. It does not imply that every domain identifier, such as `WorkflowDefinition.ID`, must be generated by the execution repository.
