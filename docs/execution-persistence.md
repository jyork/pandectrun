...
Runtime persistence identities should use distinct Go types so execution, step-execution, and attempt IDs cannot be accidentally interchanged:
...
The execution repository owns creation of these runtime identities. This rule does not apply to every domain identifier; for example, `WorkflowDefinition.ID` remains part of workflow-definition identity and is not generated by the execution repository.

### Execution

An `Execution` is one invocation of a particular workflow definition version. Workflow identity, version, input, creation time, and provenance are immutable after creation. Runtime status, terminal output, terminal error, and lifecycle timestamps are mutable only through defined lifecycle operations.

A failed execution must persist its `ExecutionError` so `GetExecution` can report why the execution failed. A representative field is:
...
Attempts have explicit status rather than requiring callers to infer state from timestamps and error fields. Initial attempt statuses are:
...
Repository-owned IDs and lifecycle timestamps should not appear in caller-supplied creation data. Dedicated creation types make that contract explicit and avoid ambiguous rules about whether an incoming `ID`, `CreatedAt`, or other lifecycle field is ignored, validated, or persisted.

Representative creation inputs are:
...
The repository generates runtime IDs and lifecycle timestamps when these entities are created. Exact creation fields may evolve as provenance and relationships are implemented, but repository-owned fields should remain absent from creation inputs.
...
The repository owns identity creation and lifecycle timestamps for persisted runtime entities. Callers provide creation data through dedicated `New*` types; the repository creates and returns the persistent ID. Runtime IDs should not be passed through `context.Context`, which remains reserved for cancellation, deadlines, and request-scoped values.
...
This is a design shape rather than a frozen Go API. Error return values and whether some operations are combined may evolve while implementing the in-memory repository, but the ownership rules for runtime IDs, lifecycle timestamps, and creation inputs are intentional.
...
A running `StepAttempt` that stops because cancellation takes effect should become `cancelled`. Pending steps that will no longer execute because cancellation took effect should also become `cancelled`, not `skipped`.
...
Once the in-memory repository exists, the scheduler should no longer accept an externally supplied execution ID or remain the sole owner of runtime state. It should create the execution through the repository, receive the generated `ExecutionID`, and use that ID for subsequent step and attempt persistence.

Conceptually, the current scheduler contract:

```go
Execute(ctx context.Context, executionID string, def WorkflowDefinition, input json.RawMessage) (WorkflowContext, error)
```

should move toward:

```go
Execute(ctx context.Context, def WorkflowDefinition, input json.RawMessage) (ExecutionID, error)
```

The repository is the authoritative source for execution status, output, errors, step state, attempt history, and events. Callers that need the completed or failed execution can retrieve it using the returned `ExecutionID`.

Scheduler tests should execute a workflow through the repository and assert the durable representation of what happened:

- execution status, output, and error;
- step statuses and outputs;
- attempt statuses and history;
